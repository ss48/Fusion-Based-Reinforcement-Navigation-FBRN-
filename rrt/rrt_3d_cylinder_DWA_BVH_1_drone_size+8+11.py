...
def update_cubes(num):
    global distances_covered, cube_times, waiting_at_goal, return_start_time, cube_speeds, pause_status, battery_levels
    ...
    for i, (cube, path, speed) in enumerate(zip(cube_plots, cube_positions, cube_speeds)):
        ...
        battery_consumption = base_battery_drain + (speed_battery_drain_factor * speed)
        battery_levels[i] = max(battery_levels[i] - battery_consumption, 0)  # Ensure battery doesn't go below 0
    ...
    return [line for cube in cube_plots for line in cube] + time_texts + speed_texts + distance_texts + battery_texts + nib_plots
