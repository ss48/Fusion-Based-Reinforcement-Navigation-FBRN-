...
if path is not None:
    ...
    cube_positions = alns_optimized_path  # Ensure this is the correct ALNS path to be followed by cubes
    ...
    for start_pos in initial_positions:
        path_segment = generate_collision_free_rrt(start_pos, final_goal, X, all_obstacles, max_attempts=20)

        if path_segment is None:
            print(f"Failed to find a path from {start_pos} to {final_goal}")
...
speeds = [0.5, 0.4, 0.3]  # Different speeds for each cube
...
if not alns_optimized_path:
    ...
    cube_positions = alns_optimized_path  # Set the cubes' paths to the ALNS path waypoints
...
def get_position_along_path(path, distance):
    if not path:  # Check if path is empty
        ...
        return np.array([0, 0, 0])  # Return default position to prevent errors
...
def update_cubes(num):
    ...
    # Update each cube's position along the ALNS path
    for i, (cube, path, speed) in enumerate(zip(cube_plots, [cube_positions] * len(cube_plots), speeds)):
        if not path:  # Ensure path has points
            continue
        distances_covered[i] += speed
        new_position = get_position_along_path(path, distances_covered[i])

        # Update vertices for each cube
        cube_vertices = create_cube_vertices(new_position, cube_size)
        
        # Update lines for the current cube
