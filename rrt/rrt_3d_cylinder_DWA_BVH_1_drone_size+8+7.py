    ...
    for i, (cube, path, speed) in enumerate(zip(cube_plots, cube_positions, speeds)):
        ...
        if waiting_at_goal[i]:
            if time.time() - return_start_time[i] >= 5:  # Wait for 5 seconds
                ...
                path.reverse()  # Reverse path for return journey
                print(f"Cube {i} is now returning to start.")
            else:
                continue  # Skip further processing for this cube until the wait is over

        new_position = np.array(get_position_along_path(path, distances_covered[i]))
        # Update distance covered with dynamic speed adjustment
        # Decrease speed as the cube nears its goal or another cube
        ...
        # Forward journey completion, prepare to wait at goal
        if distance_to_goal < GOAL_THRESHOLD and not waiting_at_goal[i]:
            if cube_times[cube_name]['end'] is None:  # Record only on forward journey
                cube_times[cube_name]['end'] = time.time()
            waiting_at_goal[i] = True
            return_start_time[i] = time.time()  # Record wait start time
            print(f"Cube {i} reached the goal. Waiting at goal.")
            continue  # Skip further movement for this cube
        ...
        # Only apply collision avoidance if this cube is moving
        if not waiting_at_goal[i]:
            for j, other_path in enumerate(cube_positions):
                if i != j:
                    # Skip collision avoidance if the other cube is stationary at a goal
                    if waiting_at_goal[j]:
                        continue
                    ...
                    # Apply offset if close to another cube and faster than the other
                    if distance_to_other < collision_radius * 2 and current_speed > speeds[j]:
                        offset_direction = np.cross(new_position - other_position, [0, 0, 1])
                        if np.linalg.norm(offset_direction) > 0:
                            offset_direction /= np.linalg.norm(offset_direction)

                        # Apply persistent offset direction to maintain safe distance
                        proposed_position = new_position + offset_direction * (collision_radius + OFFSET_DISTANCE)
                        if collision_free_expanded(X, new_position, proposed_position, collision_radius):
                            new_position = proposed_position
                            print(f"Cube {i} applied offset to avoid Cube {j}.")
                        else:
                            print(f"Cube {i} could not apply a collision-free offset and will maintain current path.")
