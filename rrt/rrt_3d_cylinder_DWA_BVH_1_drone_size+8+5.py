...
def update_cubes(num):
    global distances_covered, cube_times

    for i, (cube, path, speed) in enumerate(zip(cube_plots, cube_positions, speeds)):
        # Update distance covered and compute new position for each cube
        distances_covered[i] += speed
        new_position = get_position_along_path(path, distances_covered[i])

        # Check if cube reached the final goal
        distance_to_goal = np.linalg.norm(np.array(new_position) - np.array(x_final_goal))
        cube_name = f'cube{i+1}'
        
        # Only record end time if the cube has reached the final goal and hasn't already stopped
        if distance_to_goal < GOAL_THRESHOLD and cube_times[cube_name]['end'] is None:
            cube_times[cube_name]['end'] = time.time()  # Stop timer for this specific cube

        # Calculate elapsed time: If end time is recorded, display the fixed time; otherwise, continue counting
        elapsed_time = (
        ...
        # Update on-screen timer for each cube independently
