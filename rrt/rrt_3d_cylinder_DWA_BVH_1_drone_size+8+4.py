...
speeds = [2, 0.2, 0.3]  # Different speeds for each cube
...
SAFE_DISTANCE = 5.0  # Adjust as needed based on your scale
OFFSET_DISTANCE = 2.0  # Distance to offset to avoid collision


# Animation update function for cubes to follow ALNS path
def update_cubes(num):
    global distances_covered
    offset_applied = [False] * len(cube_plots)  # Track offset status for each cube
    ...
    for i, (cube, path, speed) in enumerate(zip(cube_plots, cube_positions, speeds)):
        ...
        # Check for potential collisions with other cubes
        for j, other_path in enumerate(cube_positions):
            if i != j:  # Avoid self-comparison
                ...
                # If close to another cube, apply offset to avoid collision
                if distance_to_other < cube_size * 2:  # Adjust collision threshold as needed
                    offset_direction = np.cross(new_position - other_position, [0, 0, 1])
                    offset_direction /= np.linalg.norm(offset_direction)  # Normalize

                    # Initial offset proposal
                    proposed_position = new_position + offset_direction * (cube_size + 2)

                    # Check for a collision-free offset position
                    attempts = 0
                    max_attempts = 20
                    while not X.collision_free(new_position, proposed_position) and attempts < max_attempts:
                        # Rotate the direction for a new offset proposal if needed
                        offset_direction = np.cross(offset_direction, [1, 1, 0])
                        offset_direction /= np.linalg.norm(offset_direction)
                        proposed_position = new_position + offset_direction * (cube_size + 2)
                        attempts += 1

                    # Apply offset if safe; otherwise, continue on the ALNS path
                    if X.collision_free(new_position, proposed_position):
                        new_position = proposed_position
                        offset_applied[i] = True
                    else:
                        print(f"Cube {i} could not find a collision-free offset. Continuing on path.")

        # Revert to ALNS path if offset was applied previously and no longer needed
        if offset_applied[i] and distance_to_other >= cube_size * 2:
            offset_applied[i] = False
    ...
    return [line for cube in cube_plots for line in cube]


# Set `frames` based on the longest path
...
#ani = animation.FuncAnimation(fig, update_cubes, frames=len(paths[0]), interval=100, blit=True)
ax.legend(['Cube 1', 'Cube 2', 'Cube 3'])
