...
x_third_goal = (-120, 160, 50)
x_final_goal = (-150, -100, 150)
...
obstacles = [
    (-50, 60, -150, 285, 35),  #(center coordinate, height,diameter)
    (-80, 20, 0, 160, 39),   
    (70, 30, 0, 190, 25),   
    (-80, -180, 50, 170, 22), 
    (0, -50, -30, 80, 32),
...
def direct_connection(path, X, all_obstacles, r=1):
    ...
    # Check if we can directly connect x_a to x_b without collision
    if X.collision_free(x_a, x_b, r):
        # Use np.vstack to concatenate as NumPy arrays to avoid broadcasting issues
        new_path = np.vstack([np.array(new_path[:i + 1]), np.array(new_path[j:])])
...
def segment_change(path, X, all_obstacles, r=1):
    ...
    new_point = list(new_point_spatial) + list(x_a[3:])  # Ensure new_point has the same dimensions as path points

    # Check if the path segment is collision-free
    if X.collision_free(spatial_x_a, new_point_spatial, r) and X.collision_free(new_point_spatial, spatial_x_b, r):
        # Use np.vstack for concatenation to handle array dimensions correctly
        new_path = np.vstack([np.array(path[:i + 1]), np.array([new_point]), np.array(path[j:])])
        return new_path
...
def detour_addition(path, X, all_obstacles, r=1):
    ...
    path_dim = len(x_a)  # Determine the dimension of path elements
    spatial_x_a = np.array(x_a[:3], dtype=np.float64)
    spatial_x_b = np.array(x_b[:3], dtype=np.float64)

    # Calculate direction and offset
    direction = spatial_x_b - spatial_x_a
    ...
    offset_vector = np.cross(direction, [0.0, 0.0, 1.0])
    norm_offset_vector = np.linalg.norm(offset_vector)

    # Handle case where offset vector could be zero if direction is vertical
    if norm_offset_vector != 0:
        offset_vector = (offset_vector / norm_offset_vector) * np.random.uniform(0.5 * r, 1.5 * r)
    else:
        offset_vector = np.array([0.0, 0.0, 0.0])

    detour_point_3d = spatial_x_a + 0.5 * (spatial_x_b - spatial_x_a) + offset_vector

    # Match detour point dimensions to path elements
    detour_point = list(detour_point_3d)
    if len(detour_point) < path_dim:
        detour_point.extend([0] * (path_dim - len(detour_point)))  # Pad with zeros if necessary
    elif len(detour_point) > path_dim:
        detour_point = detour_point[:path_dim]  # Trim to match the dimension of path elements

    # Check collision-free constraints
    if X.collision_free(spatial_x_a, detour_point_3d, r) and X.collision_free(detour_point_3d, spatial_x_b, r):
        # Ensure each part of the path being concatenated has consistent dimensions
        new_path = np.vstack([np.array(path[:i + 1]), np.array([detour_point]), np.array(path[i + 1:])])
        return new_path
...
def smooth_path_with_spline(path, num_points=100):
    """
    Smooths the given path using cubic spline interpolation.
    """
    path = np.array(path)
    t = np.linspace(0, 1, len(path))  # Parameterize path
    x_spline = CubicSpline(t, path[:, 0])
    y_spline = CubicSpline(t, path[:, 1])
    z_spline = CubicSpline(t, path[:, 2])

    # Generate smoothed path with finer sampling
    t_fine = np.linspace(0, 1, num_points)
    smoothed_path = np.vstack([x_spline(t_fine), y_spline(t_fine), z_spline(t_fine)]).T
...
# Helper function to ensure consistent shape for each segment
def ensure_consistent_shape(segments):
    """
    Ensures all segments have the same dimensionality.
    Pads or slices each segment to have the same number of columns.
    """
    max_cols = max(segment.shape[1] for segment in segments)
    processed_segments = []
    
    for segment in segments:
        # If segment has fewer columns, pad with zeros; if more, slice to match max_cols
        if segment.shape[1] < max_cols:
            padded_segment = np.hstack([segment, np.zeros((segment.shape[0], max_cols - segment.shape[1]))])
        else:
            padded_segment = segment[:, :max_cols]
        processed_segments.append(padded_segment)
        
    return processed_segments

# Ensure each segment has the same shape
segments = [np.array(alns_optimized_segment1), 
            np.array(alns_optimized_segment2[1:]), 
            np.array(alns_optimized_segment3[1:]), 
            np.array(alns_optimized_segment4),
            np.array(alns_optimized_segment1)]

# Adjust each segment to have consistent dimensions
consistent_segments = ensure_consistent_shape(segments)

# Concatenate all segments into one final path
alns_optimized_path = np.vstack(consistent_segments)

# Ensure all segments only contain (x, y, z) coordinates for concatenation
alns_optimized_segment1 = np.array(alns_optimized_segment1)[:, :3]
alns_optimized_segment2 = np.array(alns_optimized_segment2)[:, :3]
alns_optimized_segment3 = np.array(alns_optimized_segment3)[:, :3]
alns_optimized_segment4 = np.array(alns_optimized_segment4)[:, :3]

# Concatenate the path segments
alns_optimized_path = np.vstack([
    alns_optimized_segment1,
    alns_optimized_segment2[1:],  # Exclude the first point to avoid duplication
    alns_optimized_segment3[1:],  # Exclude the first point
    alns_optimized_segment4[1:],  # Exclude the first point
])
...
# Apply spline smoothing to the optimized ALNS path
smoothed_alns_path = smooth_path_with_spline(alns_optimized_path, num_points=200)

# Now, smoothed_alns_path will contain a curved path optimized by ALNS and further smoothed
...
alns_optimized_path = [x_init, x_intermediate, x_second_goal, x_third_goal, x_final_goal,x_init]  # Placeholder for testing
...
if ml_path is not None:
    ax.plot(ml_path[:, 0], ml_path[:, 1], ml_path[:, 2], color='pink', label="ML Path")

# Ensure smoothed_alns_path is defined and non-empty
if smoothed_alns_path is not None and len(smoothed_alns_path) > 1:
    # Plot the smoothed ALNS path using spline with a unique color and style
    ax.plot(smoothed_alns_path[:, 0], smoothed_alns_path[:, 1], smoothed_alns_path[:, 2], 
            color='black', linestyle='-', linewidth=2, label="Spline-Smoothed ALNS Path")
else:
    print("Warning: Spline-smoothed path is not available or has insufficient points.")
...
# Time annotations for each cube
time_texts = [
...
battery_texts = [
    ax.text2D(0.325, 0.98 - i * 0.05, "", transform=ax.transAxes)
    for i in range(3)
]
...
battery_levels = [100.0, 100.0, 100.0]  # Start with 100% battery for each cube

# Battery consumption parameters
base_battery_drain = 0.01  # Base battery drain per frame
speed_battery_drain_factor = 0.05  # Additional drain factor based on speed

# Update the `update_cubes` function
# Initialize a flag to track if the cube is moving
cube_is_moving = [True] * len(cube_plots)  # Initialize as moving

# Initialize time tracking with elapsed time field for each cube
cube_times = {
    'cube1': {'start': time.time(), 'elapsed': 0.0, 'end': None},
    'cube2': {'start': time.time(), 'elapsed': 0.0, 'end': None},
    'cube3': {'start': time.time(), 'elapsed': 0.0, 'end': None}
}

# Inside update_cubes function

def update_cubes(num):
    global distances_covered, cube_times, waiting_at_goal, return_start_time, cube_speeds, pause_status, cube_is_moving
    ...
    for i, (cube, path, speed) in enumerate(zip(cube_plots, cube_positions, cube_speeds)):
        ...
        # Check if cube has reached the goal
        if waiting_at_goal[i]:
            if cube_times[cube_name]['end'] is None:
                cube_times[cube_name]['end'] = current_time  # Set the end time when reaching the goal
            # No need to update further for this cube once it has reached the goal
            continue
        ...
        if pause_status[i]:
            if any(
                ...
                cube_speeds[i] = original_speeds[i]
                print(f"{cube_name} resuming movement after pause.")
            else:
        ...
        if distance_step > 1e-5:  # Threshold to check if cube is actually moving
            if not cube_is_moving[i]:  # If it was previously stopped, update start time
                cube_times[cube_name]['start'] = current_time
            cube_is_moving[i] = True
        else:
            cube_is_moving[i] = False

        # Update elapsed time only if the cube is moving
        if cube_is_moving[i]:
            # Add the elapsed time since the last recorded `start` time
            cube_times[cube_name]['elapsed'] += current_time - cube_times[cube_name]['start']
            # Reset the start time to now for the next increment
            cube_times[cube_name]['start'] = current_time

            # Update cumulative distance
            distances_covered[i] += distance_step

        # Adjust speed near goal and for avoidance
        ...
        if not is_returning[i] and not paused_at_goal[i]:  # Moving towards the goal
            ...
            if distance_to_goal < GOAL_THRESHOLD:
                ...
                print(f"{cube_name} reached the goal, pausing before return.")
            ...
            if current_time - return_start_time[i] >= RETURN_PAUSE_DURATION:
                ...
                print(f"{cube_name} starting return journey to start position.")
            ...
            if i != j:
                ...
                if distance_to_other < STOP_DISTANCE:
                    if priorities[i] < priorities[j]:  # Cube `i` has lower priority
                        ...
                        cube_speeds[i] = 0.0
                        pause_status[i] = True
                        print(f"{cube_name} paused to avoid collision with Cube {j+1}.")
                        break
                    else:
                    ...
                    if priorities[i] < priorities[j]:  # Cube `i` has lower priority than Cube `j`
                        ...
                        cube_speeds[i] = max(cube_speeds[i] - PAUSE_SPEED_REDUCTION_RATE, 0.05)
                        print(f"{cube_name} gradually slowing down to avoid Cube {j+1}")
                ...
                if distance_to_other < SAFE_DISTANCE and not offset_applied:
                    ...
                    if np.linalg.norm(perpendicular_offset) > 0:
                        ...
                        new_position += perpendicular_offset
                        offset_applied = True
                        print(f"{cube_name} applied lateral offset to avoid Cube {j+1}.")

        # Update cube's position based on its (possibly adjusted) speed
        if not pause_status[i]:  # Only update if not paused
            print(f"{cube_name} new position: {new_position}")

        # Check for NaN in position
        if np.any(np.isnan(new_position)):
            print(f"Warning: NaN detected in position for {cube_name}. Skipping update.")
            continue
        ...
        if distance_to_goal < GOAL_THRESHOLD:
            ...
            return_start_time[i] = current_time  # Start countdown to return
            if cube_times[cube_name]['end'] is None:
                cube_times[cube_name]['end'] = current_time  # Record end time
            print(f"{cube_name} reached the goal. Waiting at goal.")
            continue
        
        # Display time and cumulative distance for the cube
        elapsed_time = cube_times[cube_name]['elapsed']  # Only display accumulated elapsed time
        ...
        battery_texts[i].set_text(f"Cube {i + 1}-Battery: {battery_levels[i]:.0f}%")
