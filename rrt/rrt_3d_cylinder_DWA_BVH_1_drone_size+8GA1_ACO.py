...
from rrt_algorithms.ACO_algorithm.ACO import AntColony
...
def evaluate_fitness(path, obstacles):
    ...
    fitness = 1.0 / (path_length(path) + 1)  # Inverse of path length
    clearance = min_obstacle_clearance(path, obstacles)
    if clearance < 0:
        fitness -= 10  # Penalize paths that are inside obstacles
    return fitness
...
def mutate(path, mutation_rate=0.01):
    """ Mutate the path randomly. """
    for i in range(len(path)):
        if np.random.rand() < mutation_rate:
            path[i] += (np.random.rand(3) - 0.5) * 10  # Small random change
    return path

def genetic_algorithm(initial_path, obstacles, population_size=50, generations=100):
